...
class Feature:
    ...
    def from_json(cls, json_obj: dict) -> Feature:
        ...
        feature.input_parameters = [
            MyProperty.from_json(p, feature.name)
            for p in list(feature.input_parameters)
        ]
        if feature.coefficients is not None:
            feature.coefficients = MyProperty.from_json(
                feature.coefficients, feature.name
            )
        return feature
