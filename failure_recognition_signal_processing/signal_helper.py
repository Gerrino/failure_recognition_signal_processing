...
def __get_bin_search_dir(
    fun_x: Callable, y_target: float, x_i: float, pos_slope: bool, epsilon: float
) -> int:
...
def binary_search(
    ...
    low, high = xi_range_0
    if high == low:
        low, high = cond_max(low - 0.5 * abs(low) - 1, xi_range[0]), cond_min(
            2 * abs(high) + 1, xi_range[1]
        )
    for _ in range(max_iter):
        ...
        if bin_eval == 1:
            ...
            if low not in checked_boundaries:
                ...
                if (
                    __get_bin_search_dir(fun_x, y_target, high, pos_slope, epsilon)
                    == -1
                ):
...
def __find_peaks_feature(x, param: Dict[str, str]):
    ...
    for i in range(len(xf) // chunk_size):
        part_xf, part_yyf = (
            xf[i * chunk_size : (i + 1) * chunk_size],
            yyf[i * chunk_size : (i + 1) * chunk_size],
        )
        peak_x, peak_y = find_signal_peaks(
            part_xf,
            part_yyf,
            num_peaks=sub_peaks,
            mode=mode,
        ...
        peak_x, peak_y = (
            0 if len(peak_x) == 0 else peak_x[0],
            0 if len(peak_y) == 0 else peak_y[0],
        )
...
def find_peaks_feature(x, param: list):
    ...
    param_peaks_coeffis = [
        (ast.literal_eval(p), __find_peaks_feature(x, ast.literal_eval(p)), cs)
        for p, cs in distinct_params_coeff.items()
    ]

    if (
        zero_peaks_cnt := sum(
            1 for (p, pks, cs) in param_peaks_coeffis for c in cs if c >= len(pks)
        )
    ) > 0:
        print(f"Peaks index out of bounds. Filling with {zero_peaks_cnt} zeros!")
    param_peaks_coeffis = [
        (p, pks + [0] * (len(cs) - len(pks)), cs)
        for (p, pks, cs) in param_peaks_coeffis
    ]

    output_list = [
        (convert_to_output_format(p.update({"coeff": c}) or p), pks[c])
        for (p, pks, cs) in param_peaks_coeffis
        for c in cs
    ]
...
def find_signal_peaks(
    ...
    opt = binary_search(
        lambda p: len(
            find_peaks(signal, **(x_0.update({mode.value["name"]: p}) or x_0))[0]
        ),
        mode.value["range"],
        num_peaks,
        0,
